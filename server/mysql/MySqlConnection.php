...
class MySqlConnection
{
    ...
    public function orderBy($cols)
    {
        if (is_array($cols)) {
            ...
            return $this->addOrderByStr($cols);
        }
        return $this;
    }
    ...
    {
        foreach ($spec as $col => $order) {
            $this->order_by[] = $this->quoteNamesIn($col) . ' ' . $order;
        }
        $this->order_by_str = '';
        return $this;
    }

    protected function addOrderByStr(string $spec)
    {
        $this->order_by_str = $spec;
        $this->order_by = [];
        return $this;
    }
    ...
    {
        if (empty($this->order_by) && empty($this->order_by_str)) {
            return '';
        }

        if (!empty($this->order_by_str)) {
            return ' ORDER BY ' . trim($this->order_by_str);
        } elseif (!empty($this->order_by) && is_array($this->order_by)) {
